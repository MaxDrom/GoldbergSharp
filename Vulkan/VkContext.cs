...
public class VkContext : IDisposable
{
    ...
    public Silk.NET.SPIRV.Reflect.Reflect Reflect { get; } = Silk.NET.SPIRV.Reflect.Reflect.GetApi();
        ...
        void* pUserData)
    {
        ...
        var copy = $"{message}";
        var colors =
            new Dictionary<DebugUtilsMessageSeverityFlagsEXT,
                ConsoleColor>
            {
        ...
        if (copy.Trim().Length <= 0) return Vk.False;
